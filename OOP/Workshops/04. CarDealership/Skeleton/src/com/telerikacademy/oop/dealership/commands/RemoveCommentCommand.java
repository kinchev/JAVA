...
public class RemoveCommentCommand extends BaseCommand {
    ...
    private String removeComment(int vehicleIndex, int commentIndex, String username) {
        ...
        ValidationHelpers.validateIntRange(vehicleIndex, 0, user.getVehicles().size(), VEHICLE_INDEX_OUT_OF_BOUNDS);
        ValidationHelpers.validateIntRange(commentIndex, 0, user.getVehicles().get(vehicleIndex).getComments().size(), COMMENT_INDEX_OUT_OF_BOUNDS);
