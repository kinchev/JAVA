...
public class ShowCategoryCommand_Tests {
    @ParameterizedTest(name = "with size {0}")
    @ValueSource(ints = {ShowCategoryCommand.EXPECTED_NUMBER_OF_ARGUMENTS - 1, ShowCategoryCommand.EXPECTED_NUMBER_OF_ARGUMENTS + 1})
    public void should_throwException_when_argumentCountDifferentThanExpected(int testSize) {
        // Arrange
        Command command = new ShowCategoryCommand(TestUtilities.initializeRepository());
